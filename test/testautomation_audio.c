...
int audio_resampleLoss()
{
  ...
  for (int spec_idx = 0; test_specs[spec_idx].time > 0; ++spec_idx) {
    ...
    const int len_in = frames_in * sizeof (float);
    const int len_target = frames_target * sizeof (float);
    ...
    for (int i = 0; i < frames_in; ++i) {
      *(((float *) cvt.buf) + i) = sine_wave_sample(i, spec->rate_in, spec->freq, spec->phase);
    }
